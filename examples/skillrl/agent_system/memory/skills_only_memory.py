    ...
    * ``"embedding"`` – the task description is encoded with a
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_device:     Device for the embedding model in embedding
                                  mode (e.g. ``"cpu"``, ``"cuda"``, ``"npu"``).
                                  Defaults to ``"cpu"``. On Ascend NPU pass
                                  ``"npu"`` (requires torch_npu). Template mode
                                  ignores this.
        """
        ...
        # retrieve() call is not slower than subsequent ones. If the embedding
        # model cannot be loaded (e.g. no network to fetch it, or NPU device
        # unsupported by sentence-transformers), degrade gracefully to
        # template mode so the rest of the workflow still runs.
        if retrieval_mode == "embedding":
            try:
                self._compute_skill_embeddings()
            except Exception as e:  # noqa: BLE001
                print(
                    f"[SkillsOnlyMemory] Embedding model unavailable ({e}); "
                    f"falling back to template mode."
                )
                self.retrieval_mode = "template"
    ...
    def _get_embedding_model(self):
        ...
        On Ascend NPU, pass ``embedding_device="npu"`` (requires torch_npu). If
        the requested device fails to load the model, we fall back to CPU so the
        skill bank keeps working — embedding is a one-shot startup cost, not a
        per-step hot path, so CPU is an acceptable degradation.
        """
        if self._embedding_model is None:
            ...
            print(f"[SkillsOnlyMemory] Loading embedding model: {self.embedding_model_path} on {device}")
            try:
                ...
                if device == "cpu":
                    raise  # CPU failed too — propagate
                print(
                    f"[SkillsOnlyMemory] Failed to load embedding model on "
                    f"{device} ({e}); falling back to CPU."
                )
