#!/bin/bash
# SkillRL on verl v0.7.1 (NPU) — WebShop task, template skill retrieval, GRPO.
#
# Adapted from SkillRL examples/grpo_trainer/run_webshop_skills.sh for the
...
#   - WebShop envs are built in-process by env_manager.make_envs ->
#     build_webshop_envs -> WebshopEnvs -> gym.make('WebAgentTextEnv-v0').
#   - WebAgentTextEnv uses an in-memory SimServer/SimBrowser (Flask
#     test_request_context, NOT a real HTTP server) — no flask server to start.
#   - One-time prerequisite: agent_system/environments/env_package/webshop/
#     setup.sh -d all  (downloads product JSON into webshop/data/).
#
...
#
# Prerequisites:
#   - export MODEL_PATH=<your SFT checkpoint> (e.g. Jianwen/Webshop-7B-SFT)
#   - run agent_system/environments/env_package/webshop/setup.sh -d all once
#   - run examples/data_preprocess/prepare.py first to make the placeholder
#     parquet at ~/data/verl-agent/text/
set -x

ENGINE=${1:-vllm}
shift  # Remove first argument so $@ only contains extra params
export MODEL_PATH=${MODEL_PATH:-Qwen/Qwen2.5-7B-Instruct}
export WANDB_NAME="webshop_skillrl_qwen2.5_7b"

num_cpus_per_env_worker=0.1  # CPU per webshop env worker
...
# Placeholder parquet: only indicates modality + size; real shopping tasks come
# from the webshop env at reset time. Generates ~/data/verl-agent/text/{train,test}.parquet
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=webshop_skillrl_qwen2.5_7b \
